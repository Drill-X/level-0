...
public class robot_freaind {
	...
	public robot_freaind() {

		// 1. Create and initialize 2 robots.
		Robot robot = new Robot("batman");
		Robot lennard = new Robot();
		// 2. fill in the steps(a,b, and c) in the setupRobot method.

		// 3. call the setupRobot method using your two robots.
		setupRobot(lennard, Color.blue);
		setupRobot(robot, Color.magenta);
		// 4. Move robots back to the top-right and bottom-left corners
		robot.moveTo(1200, 30);
		lennard.moveTo(xPosLeft, yPosBottom); // 5. do steps 5-6 repeatedly.
		for (int i = 0; g == 0; i++) {
			seekY(robot, lennard);
			seekX(robot, lennard);
			if (foundFriend(robot, lennard)) {
				g = 1;
				robot.sparkle();
				lennard.sparkle();
				for (int j = 0; j < 5; j++) {
